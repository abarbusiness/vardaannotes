...
base_dir = r"c:\Users\Ankit Raj Sharma\Desktop\VARDAAN NOTES\vardaannotes\ICSE\Class 9\Chemistry"
...
def download_chapter():
    ...
    links = re.findall(r'href="(.*?icse-class-9-chemistry-chapter-\d+.*?)"', html)
    unique_links = []
    for link in links:
        if link not in unique_links:
            unique_links.append(link)
            
    unique_links = [l for l in unique_links if '/download-book/icse-class-9-chemistry-chapter-' in l]
    unique_links.sort()
    ...
    for i, link in enumerate(unique_links):
        ...
        try:
            with urllib.request.urlopen(pdf_req) as response:
                ...
                pdf_path = os.path.join(base_dir, folder, f"Chemistry_Chapter_{i+1}.pdf")
